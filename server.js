...
async function salvarMensagemDB(p) {
  ...
  try {
    ...
    await pool.query('INSERT INTO messages(id,conv_id,text,from_me,msg_time,msg_date,status,timestamp) VALUES($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT DO NOTHING',
  ...
  setTimeout(async () => {
    try {
      ...
      await evo.post(`/webhook/set/${INSTANCE_NAME}`, {
        url: `${host}/webhook`, webhook_by_events: false, webhook_base64: false,
        events: ['MESSAGES_UPSERT','MESSAGES_UPDATE','CONNECTION_UPDATE','SEND_MESSAGE']
      });
      console.log('✅ Webhook auto-configurado');
    } catch(e) { console.log('⚠️ Webhook auto-config:', e.message); }
  }, 3000);
});
